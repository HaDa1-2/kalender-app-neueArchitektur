...
function renderCalendarConfig(){const root=$('#calendarConfig');root.innerHTML='';state.calendars.forEach((cal,i)=>{const visible=cal.visible!==false;const div=document.createElement('div');div.className='calendar-pane'+(!visible?' hidden-cal':'');div.dataset.calIndex=i;div.innerHTML=`<div class="pane-head"><div class="cal-title-line"><button class="cal-title-click" data-edit-cal="${i}" title="Kalendername ändern"><span class="pane-name">${escapeHtml(cal.name)}</span><span class="cal-edit-icon">✎</span></button></div><div class="pane-actions"><span class="order-stack"><button class="btn small ui-icon-btn order-btn" data-move-cal="${i}:-1" title="Nach oben" ${i===0?'disabled':''}>▲</button><button class="btn small ui-icon-btn order-btn" data-move-cal="${i}:1" title="Nach unten" ${i===state.calendars.length-1?'disabled':''}>▼</button></span><button class="btn small ui-icon-btn visibility-btn ${visible?'':'hidden-state'}" data-toggle-cal="${i}" title="${visible?'Kalender ausblenden':'Kalender einblenden'}">${iconEye(visible)}</button><button class="btn small add-source-plus" data-add-source="${i}" title="Kalenderquelle hinzufügen"><span>+</span></button><button class="btn small ui-icon-btn trash-unified" data-delete-cal="${i}" title="Kalender löschen">${iconTrash()}</button></div></div>${cal.links.length?cal.links.map((l,idx)=>{const lv=l.visible!==false;const isOwn=l.type==='own';return `<div class="ics-item ${lv?'':'hidden-ics'}"><div class="ics-click" title="${isOwn?'Eigener Kalender':'ICS-Kalender'}"><b title="${escapeHtml(l.name)}"><span class="ics-color-dot" style="background:${escapeHtml(l.color||state.colors.event)}"></span>${escapeHtml(shortText(l.name,34))}</b><span>${isOwn?'Eigener Kalender':'ICS-Kalender aktiv'}</span></div><div class="ics-actions"><button class="btn small ui-icon-btn visibility-btn" data-toggle-ics="${i}:${idx}" title="${lv?'Ausblenden':'Einblenden'}">${iconEye(lv)}</button><button class="btn small ui-icon-btn" data-ics-settings="${i}:${idx}" title="${isOwn?'Eigenen Kalender einstellen':'ICS-Kalender einstellen'}">${iconSettings()}</button><button class="btn small ui-icon-btn trash-unified" data-del-ics="${i}:${idx}" title="Kalenderquelle entfernen">${iconTrash()}</button></div></div>`}).join(''):'<div class="empty">Noch keine Kalenderquelle.</div>'}<div class="status">${escapeHtml(cal.status||'')}</div>`;root.appendChild(div);});
...
const add=document.createElement('div');add.className='add-calendar-box';add.innerHTML=`<div><b>Kalender hinzufügen</b><div class="hint">Neuen Kalenderbereich erstellen und anschließend Kalenderquellen hinterlegen.</div></div><button class="btn primary small" id="addCalendarBtn">+ Kalender</button>`;root.appendChild(add);
...
function renderTaskColumnConfig(){
  ...
  if(!root)return;
  ensureSettings();
  root.innerHTML=state.taskColumns.map((col,idx)=>`<div class="taskcol-item ${col.visible===false?'hidden-col':''}" data-taskcol-index="${idx}"><div class="taskcol-main"><span class="ics-color-dot" style="background:${escapeHtml(col.color)}"></span><b lang="de" title="${escapeHtml(col.name)}">${escapeHtml(col.name)}</b></div><div class="taskcol-actions"><span class="order-stack"><button class="btn small ui-icon-btn order-btn" data-move-taskcol="${idx}:-1" title="Nach oben" ${idx===0?'disabled':''}>▲</button><button class="btn small ui-icon-btn order-btn" data-move-taskcol="${idx}:1" title="Nach unten" ${idx===state.taskColumns.length-1?'disabled':''}>▼</button></span><button class="btn small ui-icon-btn visibility-btn" data-toggle-taskcol="${idx}" title="${col.visible===false?'Einblenden':'Ausblenden'}">${iconEye(col.visible!==false)}</button><button class="btn small ui-icon-btn" data-edit-taskcol="${idx}" title="Spalte bearbeiten">${iconSettings()}</button><button class="btn small ui-icon-btn trash-unified" data-delete-taskcol="${idx}" title="Spalte löschen">${iconTrash()}</button></div></div>`).join('')+`<div class="add-calendar-box"><div><b>Tagestask-Gruppe hinzufügen</b><div class="hint">Unterteilung für Tagestasks, z. B. Geschäftlich, Arbeit, Mannschaft.</div></div><button class="btn primary small" id="addTaskColumnBtn">+ Spalte</button></div>`;
...
function renderLongColumnConfig(){
  ...
  root.innerHTML=state.longColumns.map((col,idx)=>`<div class="taskcol-item ${col.visible===false?'hidden-col':''}"><div class="taskcol-main"><span class="ics-color-dot" style="background:${escapeHtml(col.color)}"></span><b title="${escapeHtml(col.name)}">${escapeHtml(col.name)}</b></div><div class="taskcol-actions"><button class="btn small ui-icon-btn visibility-btn" data-toggle-longcol="${idx}" title="${col.visible===false?'Einblenden':'Ausblenden'}">${iconEye(col.visible!==false)}</button><button class="btn small ui-icon-btn" data-edit-longcol="${idx}" title="Gruppe bearbeiten">${iconSettings()}</button><button class="btn small ui-icon-btn trash-unified" data-delete-longcol="${idx}" title="Gruppe löschen">${iconTrash()}</button></div></div>`).join('')+`<div class="add-calendar-box"><div><b>Langfristige Gruppe hinzufügen</b><div class="hint">Gruppen für langfristige Tasks, z. B. Allgemein, Geschäftlich.</div></div><button class="btn primary small" id="addLongColumnBtn">+ Gruppe</button></div>`;
...
function initConfigBlocks(){
  $$('.config-toggle').forEach(btn=>{btn.onclick=()=>{const block=btn.closest('.config-block');if(!block)return;block.classList.toggle('collapsed');const chev=block.querySelector('.config-chevron');if(chev)chev.textContent=block.classList.contains('collapsed')?'▸':'▾';};});
}
...
function updateCollapseAllButton(){
  ...
  btn.innerHTML=all?'▾ Alle ausklappen':'▸ Alle einklappen';
...
function initConfigBlocks(){
  ...
  $$('.config-toggle').forEach(btn=>{btn.onclick=()=>{
    ...
    const chev=block.querySelector('.config-chevron');if(chev)chev.textContent=block.classList.contains('collapsed')?'▸':'▾';
    updateCollapseAllButton();
    immediatePersist();
  };});
...
function renderCalendarConfig(){
  ...
  state.calendars.forEach((cal,i)=>{
    ...
    div.innerHTML=`<div class="pane-head"><div class="cal-title-line"><button class="cal-title-click" data-edit-cal="${i}" title="Kalendername ändern"><span class="pane-name">${escapeHtml(cal.name)}</span><span class="cal-edit-icon">✎</span></button></div><div class="pane-actions"><span class="order-stack"><button class="btn small ui-icon-btn order-btn" data-move-cal="${i}:-1" title="Nach oben" ${i===0?'disabled':''}>▲</button><button class="btn small ui-icon-btn order-btn" data-move-cal="${i}:1" title="Nach unten" ${i===state.calendars.length-1?'disabled':''}>▼</button></span><button class="btn small ui-icon-btn" data-collapse-cal="${i}" title="${collapsed?'Kalender aufklappen':'Kalender einklappen'}"><span class="cal-collapse-icon">${collapsed?'▸':'▾'}</span></button><button class="btn small ui-icon-btn visibility-btn ${visible?'':'hidden-state'}" data-toggle-cal="${i}" title="${visible?'Kalender ausblenden':'Kalender einblenden'}">${iconEye(visible)}</button><button class="btn small add-source-plus" data-add-source="${i}" title="Kalenderquelle hinzufügen"><span>+</span></button><button class="btn small ui-icon-btn trash-unified" data-delete-cal="${i}" title="Kalender löschen">${iconTrash()}</button></div></div><div class="calendar-source-wrap">${sources}</div><div class="status">${escapeHtml(cal.status||'')}</div>`;
...
function renderLongColumnConfig(){
  const root=$('#longColumnConfig'); if(!root)return; ensureRev033State();
  root.innerHTML=state.longColumns.map((col,idx)=>`<div class="taskcol-item ${col.visible===false?'hidden-col':''}"><div class="taskcol-main"><span class="ics-color-dot" style="background:${escapeHtml(col.color)}"></span><b title="${escapeHtml(col.name)}">${escapeHtml(col.name)}</b></div><div class="taskcol-actions"><span class="order-stack"><button class="btn small ui-icon-btn order-btn" data-move-longcol="${idx}:-1" title="Nach oben" ${idx===0?'disabled':''}>▲</button><button class="btn small ui-icon-btn order-btn" data-move-longcol="${idx}:1" title="Nach unten" ${idx===state.longColumns.length-1?'disabled':''}>▼</button></span><button class="btn small ui-icon-btn visibility-btn" data-toggle-longcol="${idx}" title="${col.visible===false?'Einblenden':'Ausblenden'}">${iconEye(col.visible!==false)}</button><button class="btn small ui-icon-btn" data-edit-longcol="${idx}" title="Gruppe bearbeiten">${iconSettings()}</button><button class="btn small ui-icon-btn trash-unified" data-delete-longcol="${idx}" title="Gruppe löschen">${iconTrash()}</button></div></div>`).join('')+`<div class="add-calendar-box"><div><b>Langfristige Gruppe hinzufügen</b><div class="hint">Gruppen für langfristige Tasks, z. B. Allgemein, Geschäftlich.</div></div><button class="btn primary small" id="addLongColumnBtn">+ Gruppe</button></div>`;
...
renderCalendarConfig=function(){
  ...
  $$('.calendar-pane').forEach(pane=>{
    ...
    if(head&&!head.querySelector('[data-collapse-cal]')){
      const btn=document.createElement('button');btn.className='btn small ui-icon-btn';btn.type='button';btn.title=cal.collapsed!==false?'Kalendergruppe ausklappen':'Kalendergruppe einklappen';btn.innerHTML=cal.collapsed!==false?'▸':'▾';btn.dataset.collapseCal=String(i);
...
renderCalendarConfig=function(){
  ...
  $$('.calendar-pane').forEach(pane=>{
    ...
    if(btn){btn.innerHTML=cal.collapsed!==false?'▸':'▾';btn.title=cal.collapsed!==false?'Kalendergruppe ausklappen':'Kalendergruppe einklappen';}
  });
};
...
renderTaskColumnConfig=function(){
  ...
  if(!root.querySelector('#overdueColorBtn')){
    ...
    box.innerHTML=`<div class="color-bucket-label"><span class="color-bucket-preview" style="background:${escapeHtml(state.colors?.overdue||defaultColors.overdue)}"></span><span>Überzogene Tasks</span></div><button class="btn small color-bucket-btn" id="overdueColorBtn" title="Farbe wählen">🪣</button>`;
    root.prepend(box);
...
(function(){
  ...
  function colorButtonHtml56(id,color,title='Farbe wählen'){
    return `<button class="btn small ui-icon-btn color-bucket-btn rev56-color-btn" id="${id}" type="button" title="${escapeHtml(title)}"><span class="rev56-bucket-dot" style="background:${escapeHtml(color)}"></span>🪣</button>`;
  }
...
(function(){
  ...
  function colorBtn57(id,color,label){
    return `<button type="button" class="btn small rev57-color-button" id="${id}" title="${escapeHtml(label)}"><span class="rev57-color-dot" style="background:${escapeHtml(color)}"></span>🪣</button>`;
  }
...
(function(){
  ...
  function rev58OpenOwnEventEditor(calIdx,evtIdx){
    ...
    del.innerHTML=(typeof iconTrash==='function')?iconTrash():'🗑';
    actions.insertBefore(del, actions.firstChild);
    modalBackdrop.style.display='flex';
    save.style.display='';
    del.onclick=async()=>{
...
(function(){
  ...
  function colorBtn60(id,color,label){return `<button type="button" class="btn small rev60-color-button" id="${id}" title="${escapeHtml(label)}"><span class="rev60-color-dot" style="background:${escapeHtml(color)}"></span>🪣</button>`;}
...
(function(){
  ...
  function colorButton(id,color,label){return `<button type="button" class="btn small rev61-color-button" id="${id}" title="${esc(label)}"><span class="rev61-color-dot" style="background:${esc(color)}"></span>🪣</button>`;}
...
(function(){
  ...
  function colorButton65(id,color,label){return `<button type="button" class="btn small rev65-color-button" id="${id}" title="${esc(label)}"><span class="rev65-color-dot" style="background:${esc(color)}"></span>🪣</button>`;}
...
(function(){
  ...
  function colorButton66(id,color,label){return `<button type="button" class="btn small rev66-color-button" id="${id}" title="${esc66(label)}"><span class="rev66-color-dot" style="background:${esc66(color)}"></span>🎨</button>`;}
